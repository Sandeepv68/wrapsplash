/**
 * Wrapsplash API wrapper v4.0.1 for Unspalsh API
...
class WrapSplashApi {
  ...
  fetchUrl = (url, method) => {
    ...
    return ajax
      .makeRequest(url, method.toLowerCase())
      .then(function (res) {
        if (res.status === 204) {
          let response = {
            status: res.status,
  ...
  updateCurrentUserProfile = (
    ...
    let _url =
      this.API_LOCATION +
      urlConfig.UPDATE_CURRENT_USER_PROFILE +
      "?" +
  ...
  getPublicProfile = (username, width, height) => {
    let url =
      this.API_LOCATION +
      urlConfig.USERS_PUBLIC_PROFILE +
      username +
      "?w=" +
  ...
  getUserPortfolio = (username) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.USERS_PORTFOLIO.replace(/:username/gi, username);
    return this.fetchUrl(url, "GET");
  };
  ...
  getUserPhotos = (
    ...
    let url =
      this.API_LOCATION +
      urlConfig.USERS_PHOTOS.replace(/:username/gi, username) +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10) +
      "&order_by=" +
      (order_by ? order_by : "latest") +
      "&stats=" +
      (stats ? stats : "false") +
      "&resolution=" +
      (resolution ? encodeURIComponent(resolution) : "days") +
      "&quantity=" +
  ...
  getUserLikedPhotos = (username, page, per_page, order_by) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.USERS_LIKED_PHOTOS.replace(/:username/gi, username) +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10) +
  ...
  getUserCollections = (username, page, per_page) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.USERS_COLLECTIONS.replace(/:username/gi, username) +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10);
  ...
  getUserStatistics = (username, resolution, quantity) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.USERS_STATISTICS.replace(/:username/gi, username) +
      "?resolution=" +
      (resolution ? encodeURIComponent(resolution) : "days") +
      "&quantity=" +
      (quantity ? quantity : 30);
  ...
  getAPhoto = (id, width, height, rect) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_A_PHOTO.replace(/:id/gi, id) +
      "?w=" +
      (width && !isNaN(width) ? +width : "") +
      "&h=" +
      (height && !isNaN(height) ? +height : "") +
      "&rect=" +
      (rect && rect.typeof === "string" ? +encodeURIComponent(rect) : "");
    return this.fetchUrl(url, "GET");
  };
  ...
  getARandomPhoto = (
    ...
    if (
      !this.availableOrientations.includes(orientation) &&
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_A_RANDOM_PHOTO +
      "?collections=" +
      (collections && !isNaN(collections)
  ...
  getPhotoStatistics = (id, resolution, quantity) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_A_PHOTO_STATISTICS.replace(/:id/gi, id) +
      "?resolution=" +
      (resolution ? encodeURIComponent(resolution) : "days") +
      "&quantity=" +
      (quantity ? quantity : 30);
  ...
  getPhotoLink = (id) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_A_PHOTO_DOWNLOAD_LINK.replace(/:id/gi, id);
    return this.fetchUrl(url, "GET");
  };
  ...
  updatePhoto = (id, location, exif) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.UPDATE_A_PHOTO.replace(/:id/, id) +
      "?" +
      (location.latitude
        ? "&location[latitude]=" + encodeURIComponent(location.latitude)
        : "") +
      (location.longitude
        ? "&location[longitude]=" + encodeURIComponent(location.longitude)
        : "") +
      (location.name
        ? "&location[name]=" + encodeURIComponent(location.name)
        : "") +
      (location.city
        ? "&location[city]=" + encodeURIComponent(location.city)
        : "") +
      (location.country
        ? "&location[country]=" + encodeURIComponent(location.country)
        : "") +
      (location.confidential
        ? "&location[confidential]=" + encodeURIComponent(location.confidential)
        : "") +
      (exif.make ? "&exif[make]=" + encodeURIComponent(exif.make) : "") +
      (exif.model ? "&exif[model]=" + encodeURIComponent(exif.model) : "") +
      (exif.exposure_time
        ? "&exif[exposure_time]=" + encodeURIComponent(exif.exposure_time)
        : "") +
      (exif.aperture_value
        ? "&exif[aperture_value]=" + encodeURIComponent(exif.aperture_value)
        : "") +
      (exif.focal_length
        ? "&exif[focal_length]=" + encodeURIComponent(exif.focal_length)
        : "") +
      (exif.iso_speed_ratings
        ? "&exif[iso_speed_ratings]=" +
          encodeURIComponent(exif.iso_speed_ratings)
        : "");
  ...
  search = (query, page, per_page, collections, orientation) => {
    ...
    let url =
      ...
      (per_page && !isNaN(per_page) ? +per_page : 10) +
      "&collections=" +
      (collections && !isNaN(collections) ? +collections : "") +
      "&orientation=" +
  ...
  listFeaturedCollections = (page, per_page) => {
    let url =
      this.API_LOCATION +
      urlConfig.LIST_FEATURED_COLLECTIONS +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10);
  ...
  listCuratedCollections = (page, per_page) => {
    let url =
      this.API_LOCATION +
      urlConfig.LIST_CURATED_COLLECTIONS +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10);
  ...
  getCuratedCollection = (id) => {
    ...
    let url =
      this.API_LOCATION + urlConfig.GET_CURATED_COLLECTION.replace(/:id/gi, id);
    return this.fetchUrl(url, "GET");
  };
  ...
  getCollectionPhotos = (id, page, per_page) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_COLLECTION_PHOTOS.replace(/:id/gi, id) +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10);
  ...
  getCuratedCollectionPhotos = (id, page, per_page) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.GET_CURATED_COLLECTION_PHOTOS.replace(/:id/gi, id) +
      "?page=" +
      (page && !isNaN(page) ? +page : 1) +
      "&per_page=" +
      (per_page && !isNaN(per_page) ? +per_page : 10);
  ...
  listRelatedCollections = (id) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.LIST_RELATED_COLLECTION.replace(/:id/gi, id);
    return this.fetchUrl(url, "GET");
  };
  ...
  createNewColection = (title, description, private_collection) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.CREATE_NEW_COLLECTION +
      "?title=" +
  ...
  updateExistingCollection = (id, title, description, private_collection) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.UPDATE_EXISTING_COLLECTION.replace(/:id/gi, id) +
      "?title=" +
      encodeURIComponent(title) +
      (description ? "&description=" + encodeURIComponent(description) : "") +
      "&private=" +
  ...
  deleteCollection = (id) => {
    ...
    let url =
      this.API_LOCATION + urlConfig.DELETE_COLLECTION.replace(/:id/gi, id);
    return this.fetchUrl(url, "DELETE");
  };
  ...
  addPhotoToCollection = (collection_id, photo_id) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.ADD_PHOTO_TO_COLLECTION.replace(
        /:collection_id/gi,
        collection_id
      ) +
      "?photo_id=" +
  ...
  removePhotoFromCollection = (collection_id, photo_id) => {
    ...
    let url =
      this.API_LOCATION +
      urlConfig.REMOVE_PHOTO_FROM_COLLECTION.replace(
        /:collection_id/gi,
        collection_id
      ) +
      "?photo_id=" +
